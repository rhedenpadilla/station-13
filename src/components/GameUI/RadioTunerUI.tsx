...
import { X, Radio, Activity, Zap, ShieldAlert } from 'lucide-react';
...
export function RadioTunerUI() {
  ...
  const hasHeardFirstSignal = useGameState((state) => state.hasHeardFirstSignal);
  ...
  // Forbidden anomalous frequency hazard (13.66 MHz)
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in text-slate-100 font-mono select-none">
      <div className="relative w-full max-w-3xl bg-[#0F172A] border-4 border-[#334155] rounded-xl shadow-2xl overflow-hidden p-6 flex flex-col gap-5">
        {/* Header bar */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          ...
          <div className="flex flex-col gap-3">
            <div className="relative rounded-lg overflow-hidden border-2 border-[#1E293B] bg-[#07111F] p-1 shadow-inner">
              ...
              />
              <div className="absolute top-2 left-3 flex items-center gap-1.5 text-[10px] text-[#39D9E6]">
          ...
          <div className="flex flex-col gap-4 bg-[#07111F]/60 p-4 rounded-lg border border-[#334155]">
            ...
            <div className="bg-[#07111F] border-2 border-[#1E293B] p-4 rounded-lg text-center shadow-inner">
              ...
              <div className="text-4xl font-black tracking-widest text-[#39D9E6] subtle-glow">
                {currentFrequency.toFixed(2)}{' '}
                <span className="text-lg font-normal text-slate-400">MHz</span>
              </div>
              <div className="text-xs text-[#F5B960] mt-1">
                {signalLocked
                  ? 'TRANSMISSION LOCKED'
                  : `TARGET CHANNEL: ${targetFrequency.toFixed(2)} MHz`}
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-xs text-slate-400 mb-1">
                <span>12.00 MHz</span>
                <span className="text-[#39D9E6] font-bold">{targetFrequency.toFixed(2)} MHz</span>
                <span>14.50 MHz</span>
              </div>
              <input
                type="range"
            ...
            <div className="grid grid-cols-4 gap-2">
              <button
                ...
                className="bg-[#1E293B] hover:bg-[#334155] text-xs py-2 rounded border border-[#334155] transition-colors"
              >
                -0.10
              </button>
              <button
        ...
        {/* Footer */}
        <div className="flex justify-between items-center text-xs text-slate-400 border-t border-[#334155] pt-3">
          <div className="flex items-center gap-2 text-[#39D9E6]">
            <Zap className="w-4 h-4 text-[#F5B960]" />
            <span>Target Frequency: {targetFrequency.toFixed(2)} MHz</span>
          </div>
          <button
            onClick={closeRadioTuner}
